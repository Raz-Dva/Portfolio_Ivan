...
function initBackground() {
    ...
    initPlugin();
    config.background.LIGHT.bounds = FSS.Vector3.create(), config.background.LIGHT.step = FSS.Vector3.create(Math.randomInRange(0.2, 1.0), Math.randomInRange(0.2, 1.0), Math.randomInRange(0.2, 1.0))
    ...
    function update() {
        ...
        if (config.background.LIGHT.autopilot) {
            ...
            FSS.Vector3.set(attractor, config.background.LIGHT.bounds[0] * ox, config.background.LIGHT.bounds[1] * oy, config.background.LIGHT.zOffset)
        }
        for (l = scene.lights.length - 1; l >= 0; l--) {
            light = scene.lights[l];
        ...
        for (v = geometry.vertices.length - 1; v >= 0; v--) {
            ...
            FSS.Vector3.set(vertex.position, config.background.MESH.xRange * geometry.segmentWidth * ox, config.background.MESH.yRange * geometry.sliceHeight * oy, config.background.MESH.zRange * offset * oz - offset);
